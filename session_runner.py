...
from exercise_video_generator import show_exercise_visual
...
def run_session(plan_file="plan.json"):
    ...
    with PoseLandmarker.create_from_options(options) as lm:
        for i, ex in enumerate(exercises, 1):
            ...
            # ── عرض الشرح المرئي قبل التمرين ──────────────────
            print(f"\n[FormaFix] Loading exercise guide...")
            should_continue = show_exercise_visual(
            ...
            if not should_continue:
                print("[FormaFix] Session ended by user.")
                break
            # ────────────────────────────────────────────────────
